<?php
  ...
  if (!isset($data->user_name)
    ...
    echo json_encode(resultMessage(0, 422, 'Please Fill in all Required Fields!', $fields));
    return;
  endif;
...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)):
    echo json_encode(resultMessage(0, 422, 'Invalid Email Address!'));
    return;
endif;

if (strlen($password) < 8):
    echo json_encode(resultMessage(0, 422, 'Your password must be at least 8 characters long!'));
    return;
endif;

if (strlen($user_name) < 3):
    echo json_encode(resultMessage(0, 422, 'Your name must be at least 3 characters long!'));
    return;
endif;
...
try {
    ...
    if ($check_email_stmt->rowCount()):
      echo json_encode(resultMessage(0, 422, 'This E-mail already in use!'));
			return;
		else:
      $insert_query = "INSERT INTO `$table_users` (user_name, email, password) VALUES(:user_name, :email, :password)";
			...
			$insert_stmt->execute();
			echo json_encode(resultMessage(1, 201, 'You have successfully registered.'));
    endif;
} catch (PDOException $e) {
    echo json_encode(resultMessage(0, 500, $e->getMessage()));
    return;
}
